...
export async function showMedicationNotification(
  ...
  const icon = '/assets/icons/android-chrome-192x192.png'
  const badge = '/assets/icons/favicon-32x32.png'
...
export async function showLowStockNotification(
  dateKey: string,
  items: Array<{ medicationName: string; daysLeft: number; stockCount: number }>,
): Promise<void> {
  if (!('Notification' in window) || Notification.permission !== 'granted') return
  if (items.length === 0) return

  const title = 'Voorraad bijna op'
  const preview = items
    .slice(0, 3)
    .map((item) => `${item.medicationName}: ${item.daysLeft}d (${item.stockCount})`)
    .join(' • ')
  const moreCount = items.length > 3 ? ` +${items.length - 3} meer` : ''
  const body = `${preview}${moreCount}. Bestel op tijd nieuwe pillen.`

  const tag = `mt-low-stock-${slotTagForNotify(dateKey)}`
  const icon = '/assets/icons/android-chrome-192x192.png'
  const badge = '/assets/icons/favicon-32x32.png'
